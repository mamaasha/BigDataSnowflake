...
2. Открываем http://localhost:8888
3. docker exec -it jupyter-notebook bash
    3.1. jupyter server list
    3.2. Копируем значение token и вставляем его на странице http://localhost:8888
4. Копируем из папки for_copy и вставляем на открывшимся сайте в главный раздел
5. Создаем папку "исходные данные" и копируем в нее все csv-файлы
6. Запускаем файл parse_files_and_load поэтапно
7. Запускаем файл pg_to_star поэтапно
8. Запускаем файл pg_to_ch поэтапно
```
